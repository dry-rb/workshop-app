...
require "admin/article_repo"

RSpec.describe Admin::ArticleRepo do
  ...
  describe "#by_pk" do
    ...
    it "returns an article by ID" do
      result = repo.by_pk(article.id)

      expect(result.title).to eq "My article"
      expect(result).to be_an Admin::Entities::Article
    end
  end
  ...
  describe "#listing" do
    ...
    it "returns all articles in created_at descending order" do
      ...
      expect(articles[0]).to be_an Admin::Entities::Article
